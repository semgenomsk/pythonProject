...
def take_money(balance, opperation_count):
    ...
    if summ_out + comission >= balance:
        ...
        logger.error(f'Недостаточно средств для вывода. ')
    else:
        if summ_out % 50 == 0:
            ...
            logger.info(f'Введена некорректная сумма.')
...
def put_money(balance, opperation_count):
    while True:
        ...
        if sum_add % 50 != 0:
            ...
            logger.info(f'Введена корректная сумма, кратной 50')
            logger.error(f'Введена некорректная сумма. ')
            continue
        elif sum_add % 50 == 0:
...
def rich_comission(balance):
    if balance > 5_000_000:
        ...
        logger.info(f'С вас сняли налог: {balance * 0.1} ')
    return balance

def print_balance(balance):
    print(f'На Вашем счету: {round(balance, 2)}')
    logger.info(f'На Вашем счету: {round(balance, 2)}')
...
while True:
    ...
    if button == '1':
        ...
        logger.error(f'Вы ввели неизвестную команду!')