...
и наличие только букв. Названия предметов должны загружаться из
файла CSV при создании экземпляра. Другие предметы в экземпляре недопустимы.
...
if __name__ == '__main__':
    ...
    sem = Student('Семенов Семен Семенович', 'subjects.csv')
    svet = Student('Павлова Светлана Семеновна', 'subjects.csv')

    # Добавляем оценки и результаты тестов для Петра
    sem.add_grade('Математика', 5)
    sem.add_grade('Математика', 5)
    sem.add_grade('Математика', 4)
    sem.add_grade('Физика', 4)
    sem.add_grade('Физика', 3)
    sem.add_test_score('Математика', 87)
    sem.add_test_score('Математика', 91)
    sem.add_test_score('Физика', 57)

    # Добавляем оценки и результаты тестов для Екатерины
    svet.add_grade('Математика', 5)
    svet.add_grade('Математика', 5)
    svet.add_test_score('Математика', 65)
    svet.add_test_score('Математика', 87)

    # Сохраняем данные в файл CSV
    sem.save_to_csv('students.csv')
    svet.save_to_csv('students.csv')
    # Выводим на печать
    print(svet)
    print(sem)